...
const AccountDetails = () => {
      ...
      if (localDraft) {
        ...
        setFormData({
          ...
          personal: {
            ...
            linkedinLabel: parsedDraft.personal?.linkedinLabel || parsedDraft.personal?.name || '',
            githubLabel: parsedDraft.personal?.githubLabel || 'GitHub Profile',
          }
        });
        setIsEditing(true);
        ...
        setFormData({
          personal: {
            ...
            linkedinLabel: personal.linkedinLabel || personal.name || '',
            github: personal.github || '',
            githubLabel: personal.githubLabel || 'GitHub Profile',
          },
  ...
  useEffect(() => {
    ...
    const autoSaveTimer = setTimeout(() => {
      ...
      const now = new Date();
      setLastSaved(
        `Draft auto-saved at ${now.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`,
      );
    }, 1000);
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      alert(
        `Error parsing resume: ${err.message}. Check your backend terminal for details.`,
      );
    } finally {
      setIsParsing(false);
  ...
  const validateForm = () => {
    ...
    if (phone && !/^\+\d{1,4}\s?\d{6,14}$/.test(phone))
      newErrors.phone = "Include country code (e.g., +92).";
  ...
  const addArrayItem = (category, emptyItem) =>
    setFormData({
      ...formData,
      [category]: [...formData[category], emptyItem],
    });
      ...
      {isEditing && (
        <Card>
          ...
          <p className={styles.subtext}>
            Upload your PDF. Gemini will extract your details automatically.
          </p>
          ...
          {isParsing && (
            <p className={styles.parsingText}>Parsing data with Gemini...</p>
          )}
        </Card>
      )}
            ...
            <div className={styles.inputRow}>
              ...
              <div className={styles.inputGroup}>
                ...
                <input
                  disabled={!isEditing}
                  name="linkedinLabel"
                  placeholder="e.g. John Doe or LinkedIn Profile"
                  value={formData.personal.linkedinLabel}
                  onChange={handlePersonalChange}
                />
                <span className={styles.helperText}>
                  This is the clickable text shown on your resume, not the URL
                  itself.
                </span>
              </div>
            </div>
            ...
            <div className={styles.inputRow}>
              ...
              <div className={styles.inputGroup}>
                ...
                <input
                  ...
                  value={formData.personal.githubLabel}
                  onChange={handlePersonalChange}
                />
                <span className={styles.helperText}>
                  This is the clickable text shown on your resume, not the URL
            ...
            <div className={styles.skillsWrapper}>
              ...
              {isEditing && (
                <div className={styles.addSkillContainer}>
                  ...
                  {errors.skill && (
                    <span className={styles.errorText}>{errors.skill}</span>
                  )}
                </div>
              )}
            </div>
            ...
            {formData.education.map((edu, idx) => (
              <div key={idx} className={styles.dynamicBlock}>
                ...
                <div className={styles.inputRow}>
                  ...
                  <div className={styles.inputGroup}>
                    ...
                    {renderDateSelector(
                      edu.endDate,
                      (newVal) =>
                        handleArrayChange("education", idx, "endDate", newVal),
                      !isEditing || (edu.graduated === false && !edu.endDate),
                    )}
                  </div>
                </div>
                <div className={styles.checkboxGroup}>
                  ...
                  />
                  <label htmlFor={`grad-${idx}`}>
                    I have graduated from this program
                  </label>
                </div>
              </div>
            ))}
                ...
                <div className={styles.inputRow}>
                  ...
                  <div className={styles.inputGroup}>
                    ...
                    {renderDateSelector(
                      exp.endDate,
                      (newVal) =>
                        handleArrayChange("experience", idx, "endDate", newVal),
                      !isEditing,
                    )}
                  </div>
                </div>
